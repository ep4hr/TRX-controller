...
void setup() {
  ...
  digitalWrite(A5,HIGH);
   // defind pin mode for Band select
...
if (i==1 ){ vfoB=rx;rx=vfoA;ModB=Mod;Mod=ModA;AntB=Ant;Ant=AntA;AgcB=Agc;Agc=AgcA; lcd.setCursor(0,0);
    lcd.print("vfoA");}
else if (i==2 ){ vfoA=rx;rx=vfoB;ModA=Mod,Mod=ModB;AntA=Ant;Ant=AntB;AgcA=Agc;Agc=AgcB; lcd.setCursor(0,0);
    lcd.print("vfoB");}
}
...
// Print and send  Mod
   ...
   if (Mod==2){ {digitalWrite(LSB,LOW) ;}lcd.print("LSB");}
    else {digitalWrite(LSB,LOW) ;}
    if (Mod==3){  {digitalWrite(CW,LOW) ;}lcd.print("CW ");}
